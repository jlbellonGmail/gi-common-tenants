...
create table if not exists tenants.tenant_subscriptions (
  ...
  contract_id uuid not null references tenants.tenant_contracts(contract_id), plan_id uuid not null references tenants.service_plans(plan_id), price_id uuid not null references tenants.service_plan_prices(price_id),
