...
No modificar otros repositorios desde este checkout. No
...
`analyst-agent`, `qa-agent` y `code-reviewer-agent` son aliases de
compatibilidad del router, no roles arquitectónicos. Los
...
circuito. Un proyecto real documenta su stack en
...
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-work-unit.ps1 `
  -Mode Feature -Slug 01-mi-feature
```

El script valida estado, reclama el ítem, crea `feature/01-mi-feature`,
el worktree bajo `../worktrees/` y `runs/01-mi-feature/`. No iniciar una unidad desde un
...
```powershell
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-work-unit.ps1 `
  -Mode Milestone -Slug nombre-del-milestone `
  -Items 01-uno,02-dos
```

El manifest `runs/milestone-<slug>/work-unit.json` es la identidad.
...
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\assess-work-unit.ps1 `
  ...
  -EvidencePath runs/01-mi-feature/assess.jsonl
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\materialize-sdd.ps1 `
  -AssessmentPath runs/01-mi-feature/assess.jsonl `
  -Objective "objetivo verificable" `
  -OutputPath runs/01-mi-feature/sdd.json
```
